			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="businessUnitId" title="${message(code: 'businessUnit.businessUnitId.label', default: 'Business Unit Id')}" />
				...
				<g:each in="${businessUnitInstanceList}" status="i" var="businessUnitInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td>${fieldValue(bean: businessUnitInstance, field: "businessUnitId")}</td>
					
						<td>${fieldValue(bean: businessUnitInstance, field: "businessUnitName")}</td>
					
						<td>${fieldValue(bean: businessUnitInstance, field: "coreChecksActive")}</td>
